// //capability/[slug].js
// import React, { useState, useEffect } from "react";
// import Head from "next/head";
// import Link from "next/link";
// import { useRouter } from "next/router";
// import {
//   getAllCapabilityCategories,
//   getAllCapabilitySubCategories,
//   getAllCapabilityCategoryCMS,
//   getAllNews,
//   getAllEvents, // Added Events API
//   IMG_URL,
// } from "../../services/authService";

// export default function CapabilityDetail() {
//   const router = useRouter();
//   const { slug } = router.query;

//   const [selectedCategory, setSelectedCategory] = useState(null);
//   const [areaOfFocusList, setAreaOfFocusList] = useState([]);
//   const [cmsData, setCmsData] = useState(null);
//   const [relatedCapabilities, setRelatedCapabilities] = useState([]);
//   const [newsList, setNewsList] = useState([]);
//   const [eventsList, setEventsList] = useState([]); // State for Events
//   const [loading, setLoading] = useState(true);
//   const [activeTab, setActiveTab] = useState("Featured"); // Tab State

//   const createSlug = (text) => {
//     if (!text) return "";
//     return text
//       .toLowerCase()
//       .trim()
//       .replace(/&/g, "and")
//       .replace(/[^a-z0-9 -]/g, "")
//       .replace(/\s+/g, "-")
//       .replace(/-+/g, "-");
//   };

//   const cleanHTML = (html) => (html ? html.replace(/&nbsp;/g, " ") : "");

//   useEffect(() => {
//     if (!slug) return;

//     async function fetchData() {
//       setLoading(true);
//       try {
//         const [catRes, subRes, cmsRes, newsRes, eventRes] = await Promise.all([
//           getAllCapabilityCategories(),
//           getAllCapabilitySubCategories(),
//           getAllCapabilityCategoryCMS(),
//           getAllNews(),
//           getAllEvents(), // Fetching Events
//         ]);

//         const categories = catRes?.data || [];
//         const currentCat = categories.find(
//           (c) => createSlug(c.categoryName) === slug,
//         );

//         if (currentCat) {
//           setSelectedCategory(currentCat);
//           const currentId = Number(currentCat.id);

//           const allSubs = subRes?.data || [];
//           let filteredSubs = allSubs.filter(
//             (sub) => Number(sub.categoryId) === currentId,
//           );

//           if (cmsRes?.success && cmsRes.data) {
//             const matchedCMS = cmsRes.data.find(
//               (cms) => Number(cms.categoryId) === currentId,
//             );
//             if (matchedCMS) {
//               setCmsData(matchedCMS);
//               if (matchedCMS.subcategoryIds) {
//                 filteredSubs = allSubs.filter((sub) =>
//                   matchedCMS.subcategoryIds.includes(Number(sub.id)),
//                 );
//               }
//             }
//           }
//           setAreaOfFocusList(filteredSubs);
//           setRelatedCapabilities(
//             categories.filter((c) => Number(c.id) !== currentId).slice(0, 10),
//           );
//         }

//         if (newsRes?.success || newsRes?.status) {
//           const newsData = newsRes.data || [];
//           setNewsList(Array.isArray(newsData) ? newsData : []);
//         }
//         if (eventRes?.success) {
//           const eventData = eventRes.data || [];
//           setEventsList(Array.isArray(eventData) ? eventData : []);
//         }
//       } catch (err) {
//         console.error("Error fetching detail data:", err);
//       } finally {
//         setLoading(false);
//       }
//     }
//     fetchData();
//   }, [slug]);

//   if (loading)
//     return (
//       <div className="p-5 text-center">
//         <div className="spinner-border text-warning"></div>
//       </div>
//     );
//   if (!selectedCategory) return null;

//   const bannerUrl = selectedCategory.bannerImage?.startsWith("uploads")
//     ? `${IMG_URL}/${selectedCategory.bannerImage}`
//     : selectedCategory.bannerImage;

//   // Logic to determine which list to show based on active tab
//   const isEventTab =
//     activeTab === "Past Events" || activeTab === "Upcoming Events";
//   const displayList = isEventTab
//     ? eventsList.slice(0, 4)
//     : newsList.slice(0, 4);

//   return (
//     <>
//       <Head>
//         <title>{selectedCategory.categoryName} | Core Law</title>
//         <link
//           rel="stylesheet"
//           href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.1/font/bootstrap-icons.css"
//         />
//       </Head>

//       {/* --- HERO SECTION --- */}
//       <div
//         className="d-flex align-items-center justify-content-center text-center text-white position-relative"
//         style={{
//           height: "420px",
//           backgroundImage: `linear-gradient(rgba(0,0,0,0.4), rgba(0,0,0,0.4)), url(${bannerUrl})`,
//           backgroundSize: "cover",
//           backgroundPosition: "center",
//         }}>
//         <div className="container">
//           <h1
//             className="display-3 mb-5"
//             style={{ fontFamily: "serif", fontWeight: "500" }}>
//             {selectedCategory.categoryName}
//           </h1>

//           <div className="d-flex justify-content-center gap-3">
//             <button
//               onClick={() => router.back()}
//               className="btn rounded-0 px-4 py-2 fw-bold text-uppercase"
//               style={{
//                 border: "1px solid #fff",
//                 color: "#fff",
//                 fontSize: "0.75rem",
//                 letterSpacing: "1px",
//               }}>
//               &lt; BACK
//             </button>

//             <Link href="/attorneys">
//               <a
//                 className="btn rounded-0 px-4 py-2 fw-bold text-uppercase"
//                 style={{
//                   border: "1px solid #fff",
//                   color: "#fff",
//                   fontSize: "0.75rem",
//                   letterSpacing: "1px",
//                 }}>
//                 MEET THE TEAM &gt;
//               </a>
//             </Link>
//           </div>
//         </div>
//       </div>

//       {/* --- MAIN CONTENT SECTION --- */}
//       <div className="container py-5">
//         <div className="row justify-content-end">
//           <div className="col-lg-9">
//             <h2
//               className="display-6 mb-4"
//               style={{ fontFamily: "serif", fontWeight: "500" }}>
//               Areas of Focus
//             </h2>

//             <div className="row row-cols-1 row-cols-md-2 g-3 mb-5">
//               {areaOfFocusList.length > 0 ? (
//                 areaOfFocusList.map((sub) => (
//                   <div key={sub.id} className="col d-flex align-items-baseline">
//                     <span
//                       style={{
//                         color: "#a67c33",
//                         marginRight: "10px",
//                         fontSize: "1.2rem",
//                       }}>
//                       •
//                     </span>
//                     <Link
//                       href={`/capability/area-detail/${createSlug(sub.subcategoryName)}`}>
//                       <a
//                         className="text-decoration-underline"
//                         style={{
//                           color: "#a67c33",
//                           fontSize: "0.95rem",
//                           fontWeight: "500",
//                         }}>
//                         {sub.subcategoryName}
//                       </a>
//                     </Link>
//                   </div>
//                 ))
//               ) : (
//                 <p className="text-muted">No specific focus areas available.</p>
//               )}
//             </div>

//             <div
//               className="mb-5"
//               style={{ fontSize: "1.05rem", lineHeight: "1.8", color: "#333" }}>
//               {cmsData ? (
//                 <div
//                   dangerouslySetInnerHTML={{
//                     __html: cleanHTML(cmsData.content),
//                   }}
//                 />
//               ) : (
//                 <div
//                   dangerouslySetInnerHTML={{
//                     __html: cleanHTML(selectedCategory.description),
//                   }}
//                 />
//               )}
//             </div>
//           </div>
//         </div>
//       </div>

//       {/* --- DYNAMIC NEWS, INSIGHTS & EVENTS (DARK SECTION) --- */}
//       <div className="py-5" style={{ backgroundColor: "#1a1a1a" }}>
//         <div className="container py-4">
//           <div className="row justify-content-end">
//             <div className="col-lg-9">
//               <h2
//                 className="display-5 text-white mb-5"
//                 style={{ fontFamily: "serif" }}>
//                 News, Insights & Events
//               </h2>

//               <div className="d-flex flex-wrap border-bottom border-secondary mb-5 pb-0 gap-4">
//                 {[
//                   "Featured",
//                   "News",
//                   "Insights",
//                   "Past Events",
//                   "Upcoming Events",
//                 ].map((tab) => (
//                   <span
//                     key={tab}
//                     onClick={() => setActiveTab(tab)}
//                     className={`fw-bold pb-2 cursor-pointer ${activeTab === tab ? "text-white border-bottom border-3" : "text-white-50"}`}
//                     style={{
//                       cursor: "pointer",
//                       borderColor:
//                         activeTab === tab ? "#00a3e0" : "transparent",
//                       fontSize: "0.9rem",
//                     }}>
//                     {tab}
//                   </span>
//                 ))}
//               </div>

//               <div className="row">
//                 {displayList.length > 0 ? (
//                   displayList.map((item, idx) => (
//                     <div key={idx} className="col-12 mb-5">
//                       <div className="d-flex align-items-center mb-2">
//                         <span
//                           className="text-uppercase small fw-bold text-white-50"
//                           style={{ letterSpacing: "1px", fontSize: "0.75rem" }}>
//                           {new Date(
//                             item.createdAt || item.startDate,
//                           ).toLocaleDateString("en-US", {
//                             month: "long",
//                             day: "2-digit",
//                             year: "numeric",
//                           })}
//                         </span>
//                         <span className="mx-2 text-white-50 small fw-bold">
//                           |
//                         </span>
//                         <span className="text-uppercase small fw-bold text-white-50">
//                           {isEventTab ? "EVENT" : "PRESS RELEASE"}
//                         </span>
//                       </div>

//                       <div className="d-flex justify-content-between align-items-start">
//                         <div className="col-lg-9 p-0">
//                           <Link
//                             href={`/${isEventTab ? "events" : "news"}/${createSlug(item.title)}`}>
//                             <a className="text-decoration-none">
//                               <h3
//                                 className="mb-2"
//                                 style={{
//                                   color: "#be9144",
//                                   fontFamily: "serif",
//                                   fontWeight: "500",
//                                   fontSize: "1.75rem",
//                                 }}>
//                                 {item.title}
//                               </h3>
//                             </a>
//                           </Link>
//                         </div>
//                         <div className="d-none d-md-flex align-items-center text-white-50 small mt-2">
//                           <div
//                             style={{
//                               width: "30px",
//                               height: "1px",
//                               backgroundColor: "#444",
//                               marginRight: "10px",
//                             }}></div>
//                           <span>
//                             {isEventTab ? "View details" : "1 min read"}
//                           </span>
//                         </div>
//                       </div>
//                     </div>
//                   ))
//                 ) : (
//                   <p className="text-white-50">
//                     No items available in this category.
//                   </p>
//                 )}
//               </div>

//               <div className="text-center mt-2">
//                 <Link href={isEventTab ? "/events" : "/news"}>
//                   <a
//                     className="text-decoration-none fw-bold"
//                     style={{ color: "#00a3e0", fontSize: "0.9rem" }}>
//                     View More +
//                   </a>
//                 </Link>
//               </div>
//             </div>
//           </div>
//         </div>
//       </div>

//       {/* --- RELATED CAPABILITIES SECTION --- */}
//       <div className="container py-5">
//         <div className="row justify-content-end">
//           <div className="col-lg-9">
//             <h2
//               className="display-5 mb-5"
//               style={{
//                 fontFamily: "serif",
//                 color: "#c0c0c0",
//                 fontWeight: "300",
//               }}>
//               Related Capabilities
//             </h2>

//             <div className="row row-cols-1 row-cols-md-2 g-3">
//               {relatedCapabilities.map((cap) => (
//                 <div key={cap.id} className="col d-flex align-items-baseline">
//                   <span style={{ color: "#a67c33", marginRight: "10px" }}>
//                     •
//                   </span>
//                   <Link href={`/capability/${createSlug(cap.categoryName)}`}>
//                     <a
//                       className="text-decoration-underline"
//                       style={{ color: "#a67c33", fontSize: "0.95rem" }}>
//                       {cap.categoryName}
//                     </a>
//                   </Link>
//                 </div>
//               ))}
//             </div>
//           </div>
//         </div>
//       </div>

//       {/* Scroll Top Button */}
//       <div className="container text-end pb-5 mt-4">
//         <div
//           onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}
//           className="d-inline-flex align-items-center justify-content-center shadow-sm"
//           style={{
//             backgroundColor: "#be9144",
//             width: "40px",
//             height: "35px",
//             cursor: "pointer",
//           }}>
//           <i className="bi bi-chevron-up text-white fs-5"></i>
//         </div>
//       </div>
//     </>
//   );
// }

import React, { useState, useEffect } from "react";
import Head from "next/head";
import Link from "next/link";
import { useRouter } from "next/router";
import {
  getAllCapabilityCategories,
  getAllCapabilitySubCategories,
  getAllCapabilityCategoryCMS,
  getAllNews,
  getAllEvents,
  IMG_URL,
} from "../../services/authService";

export default function CapabilityDetail() {
  const router = useRouter();
  const { slug } = router.query;

  const [selectedCategory, setSelectedCategory] = useState(null);
  const [areaOfFocusList, setAreaOfFocusList] = useState([]);
  const [cmsData, setCmsData] = useState(null);
  const [relatedCapabilities, setRelatedCapabilities] = useState([]);
  const [newsList, setNewsList] = useState([]);
  const [eventsList, setEventsList] = useState([]);
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState("Featured");

  const createSlug = (text) => {
    if (!text) return "";
    return text
      .toLowerCase()
      .trim()
      .replace(/&/g, "and")
      .replace(/[^a-z0-9 -]/g, "")
      .replace(/\s+/g, "-")
      .replace(/-+/g, "-");
  };

  const cleanHTML = (html) => (html ? html.replace(/&nbsp;/g, " ") : "");

  useEffect(() => {
    if (!slug) return;

    async function fetchData() {
      setLoading(true);
      try {
        const [catRes, subRes, cmsRes, newsRes, eventRes] = await Promise.all([
          getAllCapabilityCategories(),
          getAllCapabilitySubCategories(),
          getAllCapabilityCategoryCMS(),
          getAllNews(),
          getAllEvents(),
        ]);

        const categories = catRes?.data || [];
        const currentCat = categories.find(
          (c) => createSlug(c.categoryName) === slug,
        );

        if (currentCat) {
          setSelectedCategory(currentCat);
          const currentId = Number(currentCat.id);

          // Get all subcategories from database for this category
          const allSubs = subRes?.data || [];
          const dbFilteredSubs = allSubs.filter(
            (sub) => Number(sub.categoryId) === currentId,
          );

          if (cmsRes?.success && cmsRes.data) {
            const matchedCMS = cmsRes.data.find(
              (cms) => Number(cms.categoryId) === currentId,
            );
            if (matchedCMS) {
              setCmsData(matchedCMS);
              // If CMS has specific subcategories, use them, but fallback to DB ones if empty
              if (
                matchedCMS.subcategoryIds &&
                matchedCMS.subcategoryIds.length > 0
              ) {
                const cmsIds = matchedCMS.subcategoryIds.map((id) =>
                  Number(id),
                );
                // Show items that are either in CMS or just newly added to this category
                const combined = allSubs.filter(
                  (sub) =>
                    cmsIds.includes(Number(sub.id)) ||
                    Number(sub.categoryId) === currentId,
                );
                setAreaOfFocusList(combined);
              } else {
                setAreaOfFocusList(dbFilteredSubs);
              }
            } else {
              setAreaOfFocusList(dbFilteredSubs);
            }
          } else {
            setAreaOfFocusList(dbFilteredSubs);
          }

          setRelatedCapabilities(
            categories.filter((c) => Number(c.id) !== currentId).slice(0, 10),
          );
        }

        setNewsList(newsRes?.data || []);
        setEventsList(eventRes?.data || []);
      } catch (err) {
        console.error("Error fetching detail data:", err);
      } finally {
        setLoading(false);
      }
    }
    fetchData();
  }, [slug]);

  if (loading)
    return (
      <div className="p-5 text-center">
        <div className="spinner-border text-warning"></div>
      </div>
    );
  if (!selectedCategory) return null;

  const bannerUrl = selectedCategory.bannerImage?.startsWith("uploads")
    ? `${IMG_URL}/${selectedCategory.bannerImage}`
    : selectedCategory.bannerImage;
  const isEventTab =
    activeTab === "Past Events" || activeTab === "Upcoming Events";
  const displayList = isEventTab
    ? eventsList.slice(0, 4)
    : newsList.slice(0, 4);

  return (
    <>
      <Head>
        <title>{selectedCategory.categoryName} | Core Law</title>
      </Head>
      <div
        className="d-flex align-items-center justify-content-center text-center text-white position-relative"
        style={{
          height: "420px",
          backgroundImage: `linear-gradient(rgba(0,0,0,0.4), rgba(0,0,0,0.4)), url(${bannerUrl})`,
          backgroundSize: "cover",
          backgroundPosition: "center",
        }}>
        <div className="container">
          <h1 className="display-3 mb-5" style={{ fontFamily: "serif" }}>
            {selectedCategory.categoryName}
          </h1>
          <div className="d-flex justify-content-center gap-3">
            <button
              onClick={() => router.back()}
              className="btn rounded-0 px-4 py-2 fw-bold text-white border border-white"
              style={{ fontSize: "0.75rem" }}>
              &lt; BACK
            </button>
            <Link href="/attorneys">
              <a
                className="btn rounded-0 px-4 py-2 fw-bold text-white border border-white"
                style={{ fontSize: "0.75rem" }}>
                MEET THE TEAM &gt;
              </a>
            </Link>
          </div>
        </div>
      </div>

      <div className="container py-5">
        <div className="row justify-content-end">
          <div className="col-lg-9">
            <h2
              className="display-6 mb-2" // Heading ke niche ka gap kam kiya
              style={{ fontFamily: "serif", fontWeight: "500" }}>
              Areas of Focus
            </h2>

            {/* row-cols-md-2 ke saath g-1 use karne se horizontal aur vertical dono gaps kam ho jayenge */}
            <div className="row row-cols-1 row-cols-md-2 g-1 mb-4">
              {areaOfFocusList.length > 0 ? (
                areaOfFocusList.map((sub) => (
                  <div
                    key={sub.id}
                    className="col d-flex align-items-baseline py-1">
                    {/* py-1 add karne se row ke beech ka space manually control hoga */}
                    <span
                      style={{
                        color: "#a67c33",
                        marginRight: "6px", // Bullet aur text ke beech ka gap bhi 10px se 6px kar diya
                        fontSize: "1.2rem",
                      }}>
                      •
                    </span>
                    <Link
                      href={`/capability/area-detail/${createSlug(sub.subcategoryName)}`}>
                      <a
                        className="text-decoration-underline"
                        style={{
                          color: "#a67c33",
                          fontSize: "0.95rem",
                          fontWeight: "500",
                          lineHeight: "1.2", // Line height kam karne se gap aur kam lagega
                        }}>
                        {sub.subcategoryName}
                      </a>
                    </Link>
                  </div>
                ))
              ) : (
                <p className="text-muted">No specific focus areas available.</p>
              )}
            </div>
            <div
              className="mb-5"
              style={{ fontSize: "1.05rem", lineHeight: "1.8", color: "#333" }}>
              <div
                dangerouslySetInnerHTML={{
                  __html: cmsData
                    ? cleanHTML(cmsData.content)
                    : cleanHTML(selectedCategory.description),
                }}
              />
            </div>
          </div>
        </div>
      </div>

      <div className="py-5" style={{ backgroundColor: "#1a1a1a" }}>
        <div className="container py-4">
          <div className="row justify-content-end">
            <div className="col-lg-9">
              <h2
                className="display-5 text-white mb-5"
                style={{ fontFamily: "serif" }}>
                News & Events
              </h2>
              <div className="d-flex flex-wrap border-bottom border-secondary mb-5 pb-0 gap-4">
                {[
                 
                  "News",
                
                  "Upcoming Events",
                ].map((tab) => (
                  <span
                    key={tab}
                    onClick={() => setActiveTab(tab)}
                    className={`fw-bold pb-2 cursor-pointer ${activeTab === tab ? "text-white border-bottom border-3" : "text-white-50"}`}
                    style={{
                      cursor: "pointer",
                      borderColor:
                        activeTab === tab ? "#00a3e0" : "transparent",
                      fontSize: "0.9rem",
                    }}>
                    {tab}
                  </span>
                ))}
              </div>
              <div className="row">
                {displayList.map((item, idx) => (
                  <div key={idx} className="col-12 mb-5">
                    <div className="d-flex align-items-center mb-2">
                      <span className="text-uppercase small fw-bold text-white-50">
                        {new Date(
                          item.createdAt || item.startDate,
                        ).toLocaleDateString("en-US", {
                          month: "long",
                          day: "2-digit",
                          year: "numeric",
                        })}
                      </span>
                      <span className="mx-2 text-white-50 small">|</span>
                      <span className="text-uppercase small fw-bold text-white-50">
                        {isEventTab ? "EVENT" : "PRESS RELEASE"}
                      </span>
                    </div>
                    <div className="d-flex justify-content-between align-items-start">
                      <Link
                        href={`/${isEventTab ? "events" : "news"}/${createSlug(item.title)}`}>
                        <a className="text-decoration-none col-lg-9 p-0">
                          <h3 style={{ color: "#be9144", fontFamily: "serif" }}>
                            {item.title}
                          </h3>
                        </a>
                      </Link>
                      <div className="d-none d-md-flex align-items-center text-white-50 small mt-2">
                        <div
                          style={{
                            width: "30px",
                            height: "1px",
                            backgroundColor: "#444",
                            marginRight: "10px",
                          }}></div>
                        <span>
                          {isEventTab ? "View details" : "1 min read"}
                        </span>
                      </div>
                    </div>
                  </div>
                ))}
              </div>
              <div className="text-center mt-2">
                <Link href={isEventTab ? "/events" : "/news"}>
                  <a
                    className="text-decoration-none fw-bold"
                    style={{ color: "#00a3e0" }}>
                    View More +
                  </a>
                </Link>
              </div>
            </div>
          </div>
        </div>
      </div>

      <div className="container py-5">
        <div className="row justify-content-end">
          <div className="col-lg-9">
            <h2
              className="display-5 mb-5"
              style={{ fontFamily: "serif", color: "#c0c0c0" }}>
              Related Capabilities
            </h2>
            <div className="row row-cols-1 row-cols-md-2 g-3">
              {relatedCapabilities.map((cap) => (
                <div key={cap.id} className="col d-flex align-items-baseline">
                  <span style={{ color: "#a67c33", marginRight: "10px" }}>
                    •
                  </span>
                  <Link href={`/capability/${createSlug(cap.categoryName)}`}>
                    <a
                      className="text-decoration-underline"
                      style={{ color: "#a67c33" }}>
                      {cap.categoryName}
                    </a>
                  </Link>
                </div>
              ))}
            </div>
          </div>
        </div>
      </div>
    </>
  );
}