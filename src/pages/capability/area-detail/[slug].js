...
//       setLoading(true);
//       try {
//         const [subRes, cmsRes, newsRes, eventRes, catRes] = await Promise.all([
//           getAllCapabilitySubCategories(),
//           getAllSubcategoryCMS(),
//           getAllNews(),
//           getAllEvents(),
...
//             // Fetch CMS Content
//             if (cmsRes?.success && cmsRes.data) {
//               const matchedCMS = cmsRes.data.find(cms => Number(cms.subcategoryId) === currentSubId);
//               setSubCmsData(matchedCMS || null);
//             }

//             // Related Focus Areas (Same Category ki doosri sub-categories)
//             const focusItems = allSubs.filter(item => Number(item.categoryId) === parentCatId && Number(item.id) !== currentSubId);
//             setRelatedSubcategories(focusItems);

//             // Related Capabilities (Main Categories)
//             if (catRes?.data) {
//                 setRelatedCapabilities(catRes.data.filter(c => Number(c.id) !== parentCatId).slice(0, 10));
//             }
//           }
//         }
//         setNewsList(newsRes?.data || []);
...
//     ? `${IMG_URL}/${selectedSubCategory.bannerImage}` : selectedSubCategory.bannerImage;

//   const isEventTab = activeTab === "Upcoming Events";
//   const displayList = isEventTab ? eventsList.slice(0, 4) : newsList.slice(0, 4);
...
//           </h1>
//           <div className="d-flex justify-content-center gap-3">
//              <button onClick={() => router.back()} className="btn rounded-0 px-4 py-2 fw-bold text-white border border-white" style={{ fontSize: '0.75rem' }}>
//                &lt; BACK
//              </button>
//              <Link href="/attorneys">
//                <a className="btn rounded-0 px-4 py-2 fw-bold text-white border border-white" style={{ fontSize: '0.75rem' }}>
//                  MEET THE TEAM &gt;
//                </a>
//              </Link>
//           </div>
...
//               <>
//                 <h2 className="display-6 mb-2" style={{ fontFamily: "serif", fontWeight: "500" }}>
//                   Related Focus Areas
//                 </h2>
//                 <div className="row row-cols-1 row-cols-md-2 g-1 mb-4">
//                   {relatedSubcategories.map((sub) => (
//                     <div key={sub.id} className="col d-flex align-items-baseline py-1">
//                       <span style={{ color: "#a67c33", marginRight: "6px", fontSize: "1.2rem" }}>•</span>
//                       <Link href={`/capability/area-detail/${createSlug(sub.subcategoryName)}`}>
//                         <a className="text-decoration-underline" style={{ color: "#a67c33", fontSize: "0.95rem", fontWeight: "500", lineHeight: "1.2" }}>
//                           {sub.subcategoryName}
//                         </a>
//                       </Link>
//                     </div>
...
//             <div className="mb-5" style={{ fontSize: '1.05rem', lineHeight: '1.8', color: '#333' }}>
//                 <div dangerouslySetInnerHTML={{ 
//                   __html: subCmsData ? cleanHTML(subCmsData.content || subCmsData.textEditorContent) : cleanHTML(selectedSubCategory.description) 
//                 }} />
//             </div>
//           </div>
//         </div>
...
//           <div className="row justify-content-end">
//             <div className="col-lg-9">
//               <h2 className="display-5 text-white mb-5" style={{ fontFamily: 'serif' }}>News & Events</h2>
...
//                     style={{ 
//                       cursor: 'pointer', 
//                       borderColor: activeTab === tab ? "#00a3e0" : "transparent",
//                       fontSize: '0.9rem' 
//                     }}
//                    >
//                      {tab}
...
//                          {new Date(item.createdAt || item.startDate).toLocaleDateString('en-US', { month: 'long', day: '2-digit', year: 'numeric' })}
//                       </span>
...
//                         <Link href={`/${isEventTab ? "events" : "news"}/${createSlug(item.title)}`}>
//                           <a className="text-decoration-none col-lg-9 p-0">
//                             <h5 style={{ color: '#be9144', fontFamily: 'serif' }}>{item.title}</h5>
//                           </a>
//                         </Link>
//                       <div className="d-none d-md-flex align-items-center text-white-50 small mt-2">
//                         <div style={{ width: '30px', height: '1px', backgroundColor: '#444', marginRight: '10px' }}></div>
//                         <span>{isEventTab ? "View details" : "1 min read"}</span>
//                       </div>
//                     </div>
//                   </div>
//                 ))}
...
//         <div className="row justify-content-end">
//           <div className="col-lg-9">
//             <h2 className="display-5 mb-5" style={{ fontFamily: 'serif', color: '#c0c0c0' }}>
...
//                   <span style={{ color: '#a67c33', marginRight: '10px' }}>•</span>
//                   <Link href={`/capability/${createSlug(cap.categoryName)}`}>
//                     <a className="text-decoration-underline" style={{ color: '#a67c33' }}>
//                       {cap.categoryName}
//                     </a>
//                   </Link>
//                 </div>
      ...
      try {
        ...
        if (subRes?.data) {
          ...
          if (selected) {
            ...
            const sameSubs = allSubs.filter(item => 
              Number(item.categoryId) === parentCatId && Number(item.id) !== currentSubId
            );
            setSameCategorySubs(sameSubs);

            // 2. Dusri categories ki sub-categories (Bottom "Related Capabilities" section ke liye)
            const otherSubs = allSubs.filter(item => 
