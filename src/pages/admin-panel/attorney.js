...
const Attorney = () => {
  ...
  const toggleStatus = async (attorney) => {
    try {
      ...
      await authService.updateAttorney(attorney.id, formData);
      toast.success(
        `Attorney is now ${newStatus === "active" ? "Active" : "Inactive"}`,
      );
      fetchData();
    } catch (err) {
      console.error("Status Toggle Error:", err);
        ...
        <tbody>
          {currentItems.map((u, i) => (
            <tr key={u.id}>
              ...
              {/* Added Status Column with Click Event */}
              {/* Inside currentItems.map for the status column */}
              <td className="text-center">
                <Badge
                  color={u.status === "active" ? "success" : "danger"}
                  pill
            ...
            <Col md={3}>
              <FormGroup>
                ...
                <Input
                  type="select"
                  name="status"
                  value={editData.status || "inactive"} // Default to inactive
                  onChange={handleInputChange}>
                  <option value="active">Active</option>
                  <option value="inactive">Inactive</option>{" "}
                  {/* Changed from dactive */}
                </Input>
              </FormGroup>
            </Col>
            <Col md={3}>
