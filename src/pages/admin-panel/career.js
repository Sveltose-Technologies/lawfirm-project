...
const AdminCareerManagement = () => {
  ...
  const [frontForm, setFrontForm] = useState({
    categoryId: "",
    bannerText: "", // Maps to 'content'
    countryId: "",
    bannerImage: null, // Maps to 'bannerImage' or 'image'
  });
  const [careerForm, setCareerForm] = useState({
    jobTitle: "",
    jobCode: "",
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setLawCategories(extract(lawRes));
      setCareerLaws(extract(careerLawRes));
      setCareerDetails(extract(detailRes));
      setJobCategories(extract(jobCatRes));
      setCareers(extract(careerRes));
      setCountries(extract(countryRes));
      setCareerBanners(extract(bannerRes));
    } catch (error) {
      toast.error("Failed to load data");
    } finally {
      setLoading(false);
  ...
  const handleAction = async (e) => {
    ...
    try {
      ...
      if (modal.type === "front" && activeSubTab === "Banner") {
        ...
        // Career Law (Law Students, etc.) Logic
        fd.append("categoryid", frontForm.categoryId || "");
        fd.append("countryId", frontForm.countryId || "");
        fd.append("content", frontForm.bannerText || "");
        if (frontForm.bannerImage instanceof File) {
          fd.append("image", frontForm.bannerImage);
        }
        res = modal.edit
      ...
      toast.error(
        error.response?.data?.message ||
          "Operation failed. Server error (500).",
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async (type, id) => {
    ...
    try {
      if (modal.type === "front" && activeSubTab === "Banner") {
        await authService.deleteCareerBanner(id);
      } else if (modal.type === "front") {
        await authService.deleteCareerLaw(id);
      } else if (type === "lawCat") {
        await authService.deleteLawCareerCategory(id);
      } else if (type === "jobCat") {
        await authService.deleteJobCategory(id);
      } else if (type === "detail") {
        await authService.deleteCareerDetail(id);
      } else if (type === "career") {
  ...
  return (
    <Container fluid className="p-2 p-md-4 bg-light-gray min-vh-100">
      ...
      <TabContent activeTab={activeTab}>
        ...
        {/* TAB 2: CAREER FRONT (Integrated Banner & Career Law) */}
        <TabPane tabId="2">
          <Nav pills className="mb-3 bg-white p-2 rounded border shadow-sm">
            {["Banner", "Law Students", "Attorneys", "Professional Staff"].map(
              (sub) => (
            ...
            <Table hover responsive className="align-middle mb-0">
              ...
              <tbody>
                {(activeSubTab === "Banner"
                  ...
                  : careerLaws.filter(
                      (item) =>
                        getCatName(item.categoryid || item.categoryId) ===
                        activeSubTab,
                    )
                ).map((item) => (
                  <tr key={getItemId(item)}>
