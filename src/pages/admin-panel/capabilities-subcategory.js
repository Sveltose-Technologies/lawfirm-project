...
const CapabilitySubCategory = () => {
  ...
  const getCategoryName = (id) => {
    // String aur Number dono ko handle karne ke liye String() use kiya
    const found = parentCategories.find((cat) => String(cat.id) === String(id));
    return found ? found.categoryName : `ID: ${id}`;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const dataToSend = new FormData();
      dataToSend.append("adminId", "1");
      dataToSend.append("categoryId", formData.categoryId); // 👈 Yeh ID check karni hai
      dataToSend.append("subcategoryName", formData.subcategoryName.trim());
      dataToSend.append("description", formData.description);
      ...
      console.log("📤 Sending CategoryId:", formData.categoryId);
      ...
      if (res && res.success) {
        alert("Success!");
        await fetchData(); // 🔄 Backend se naya data fetch karein taaki Parent Category refresh ho
        toggle();
      } else {
  ...
  const handleEdit = (item) => {
    console.log("✏️ Editing Item CategoryId:", item.categoryId);
    setFormData({
      categoryId: String(item.categoryId), // 👈 Number ko String banaya dropdown ke liye
      subcategoryName: item.subcategoryName || "",
      description: item.description || "",
      bannerImage: null,
    });
  ...
  return (
    <Container
      ...
      style={{ backgroundColor: "#f9f9f9" }}>
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h4 className="fw-bold">Capability Subcategories</h4>
        <Button
          style={{ backgroundColor: GOLD, border: "none" }}
          onClick={toggle}>
          + Add New
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        if (window.confirm("Delete?")) {
                          await authService.deleteCapabilitySubCategory(
                            item.id,
                          );
                          fetchData();
                        }
      ...
      <Modal isOpen={modal} toggle={toggle} size="lg" centered>
        <ModalHeader toggle={toggle}>Subcategory Details</ModalHeader>
        <ModalBody>
          <Form onSubmit={handleSubmit}>
            <Row>
              <Col md={6}>
                <FormGroup>
                  ...
                  <Input
                    type="select"
                    value={formData.categoryId} // 👈 State se bind hai
                    onChange={(e) =>
                      setFormData({ ...formData, categoryId: e.target.value })
                    }
                    required>
