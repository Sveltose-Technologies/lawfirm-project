...
//     setLoading(true);
//     try {
//       const [eventRes, catRes, subRes, countRes, cityRes, attRes] =
//         await Promise.all([
...
//       data.append(
//         "capabilityCategoryId",
//         Number(formData.capabilityCategoryId),
//       );
...
//             ))
//           ) : (
//             <div className="p-2 text-white small text-center">
//               No {label} available
//             </div>
//           )}
//         </DropdownMenu>
...
const Events = () => {
  ...
  // ✨ Fixed SafeArray to handle your log structure
  const safeArray = (res) => {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      // 🚀 Using separate calls to prevent one failure from stopping everything
      ...
      console.log("✅ Data states updated");
    } catch (error) {
      console.error("Fetch Error:", error);
    } finally {
      setLoading(false);
